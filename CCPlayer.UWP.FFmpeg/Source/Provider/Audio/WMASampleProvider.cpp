#include "pch.h"
#include "WMASampleProvider.h"


WMASampleProvider::WMASampleProvider(
	FFmpegReader* reader,
...
HRESULT WMASampleProvider::CreateMediaType(IMFMediaType** ppMediaType)
{
	HRESULT hr = S_OK;
	...
	if (SUCCEEDED(hr))
	{
		if (m_pAvCodecCtx->codec_id == AV_CODEC_ID_WMAPRO)
		{
			hr = pMediaType->SetGUID(MF_MT_SUBTYPE, MFAudioFormat_WMAudioV9);
		}
		else if (m_pAvCodecCtx->codec_id == AV_CODEC_ID_WMALOSSLESS)
		{ 
			hr = pMediaType->SetGUID(MF_MT_SUBTYPE, MFAudioFormat_WMAudio_Lossless);
		}
		else 
		{
